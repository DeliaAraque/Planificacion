<?php
	...
	if($n) {
		echo "Ya existe un profesión con ese nombre&&error";
		exit;
	}
	...
	if($exe) {
		$sql = "insert into historial values('" . time() . "', '$_SESSION[nombre] $_SESSION[apellido] ($_SESSION[cedula])', 'Se modificó la profesión <strong>$nombreAnt</strong> por <strong>$nombre</strong>', '" . htmlspecialchars($sql, ENT_QUOTES) . "')";
		$exe = pg_query($sigpa, $sql);
